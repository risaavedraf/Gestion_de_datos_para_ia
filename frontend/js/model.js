...
async function loadModelMetrics() {
    ...
    if (metrics.feature_importance) renderFeatureImportance(metrics.feature_importance);
}
...
async function predictTransaction() {
    ...
    if (result && resultEl) {
        ...
        const predLabel = result.prediction === 1 ? '⚠️ FRAUDE DETECTADO' : '✅ TRANSACCIÓN LEGÍTIMA';
        const probPct = result.probability !== undefined
...
async function retrainModel() {
    ...
    try {
        const result = await api('/api/model/train', { method: 'POST' });
...
async function runDemoPrediction() {
    ...
    if (result && result.prediction !== undefined) {
        ...
        if (explanationEl) {
            const reasons = [];
            if (parseFloat(amt) > 500) reasons.push('monto elevado ($' + parseFloat(amt).toLocaleString() + ')');
            if (parseInt(hour) >= 22 || parseInt(hour) <= 5) reasons.push('horario nocturno (' + hour + 'hrs)');
            if (parseFloat(distance) > 100) reasons.push('distancia inusual (' + distance + 'km)');
            if (parseFloat(distance) > 500) reasons.push('distancia muy alta (' + distance + 'km)');
            if (['shopping_pos', 'shopping_net'].includes(category)) reasons.push('categoría de compras');
            if (parseFloat(amt) > 2000) reasons.push('monto muy alto');
            if (reasons.length === 0) reasons.push('patrones normales de transacción');
