...
_MERCHANTS = [
    ...
    "fraud_Lemke and Sons",
    "fraud_Greenfelder, Grant and Feest",
    "fraud_Kohler Inc",
    "fraud_Cassin, Harber and Murray",
    "fraud_Romaguera Ltd",
    "fraud_Schumm PLC",
    "fraud_Corwin-Hartmann",
    "fraud_O'Kon Ltd",
    "fraud_Bauch LLC",
]
...
def generate_synthetic_data(n: int = DATA_SAMPLE_SIZE) -> list[dict]:
...
def seed_data() -> None:
    ...
    Three-way gate:
    - CSV exists → skip generation, run pipeline on full data
    - CSV missing + ALLOW_SYNTHETIC_SEED=true → generate DATA_SAMPLE_SIZE rows
    - CSV missing + no env var → error exit
    """
    ...
    if RAW_CSV.exists():
        ...
        print(
            "[seed] Place the real CSV at Data/bronze/02_fraudTest.csv "
            "or set ALLOW_SYNTHETIC_SEED=true."
        )
        sys.exit(1)
